...
an element of $P$, and denote it with infix notation as $t_1=t_2$.
A first-order temporal formula over $(C,P)$ is defined by the same
...
same as that the of THT satisfaction relation defined in Subsection~\ref{subsec:tht-tel}, if
...
\begin{definition}[Safe Temporal Rule, Safe Temporal Program]
  ...
  all of its variables are safe. A universal temporal rule
  $\forallx \psi$ is safe iff $\psi$ is safe. A universal temporal
  program is safe, if all of its rules are safe.
\end{definition}
\begin{example}
...
\begin{definition}[Positive Subformula]\label{def:pos-subformula}
  For a formula $\varphi \in \langfcpt$, we define a binary
  ...
  \[
    \begin{aligned}
      ...
      \cup \{ &(\gamma, \psi) \mid
           ...
           &\oplus \in \{ \previous, \wprevious, \eventuallyP, \alwaysP, \Next, \wnext, \eventuallyF, \alwaysF \},
           \otimes \in \{ \since, \trigger, \until, \release, \wedge, \vee \} \}
    \end{aligned}
  \]
...
\begin{proposition}\label{prop:equil-iff-equil-groundc}
  ...
  $\mathcal{M} = \qttandt$ a QTHT structure. Then, $\mathcal{M}$ is a
...
its gringo representation cannot simply be done by passing the
...
not (necessarily) see any rules with $\gamma$ in its head, and will
