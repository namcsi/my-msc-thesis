...
gringo programs by serializing its abstract syntax tree (AST) into a
...
will therefore not describe the format in its entirety, and restrict
...
literals with identifier \texttt{body\_literals(11)} constituting its
body. On lines 11 and 17 we see that the tuple contains the body
literals with identifiers \texttt{body\_literal(12)} as its first
element, and \texttt{body\_literal(17)} as its second element.  A
tuple may also be empty; in this case there will be no facts encoding
its elements in the reified representation. For example, the rule
...
indicates its sign, with "pos" being positive, and "not" being
...
AST. The second argument of a function fact is its name, encoded as a
...
encoding a valid AST, and outputs its representation as a gringo
...
through recursively to reconstruct the program in its gringo
...
predicate is the name of a (temporal) operator in its gringo
...
input reified facts its arity. This is done by counting up the number
...
considered a temporal formula, namely, when its name and arity matches one
...
\texttt{safe\_atomic\_subformula/2}, which holds iff the first argument
...
as its tuple of statements. This reuse of the id is not problematic,
...
external statement and its head for head externals, while the rule on
