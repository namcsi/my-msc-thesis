...
subf(\Gamma) \defeq (&\{ subf(b_i) \mid \temprulelong \in \Gamma, \rangecc{i}{1}{n} \}\\
  \cup &\{ subf(h_j) \mid \temprulelong \in \Gamma,\rangecc{j}{1}{m} \})
\end{align*}

The set $subf(\Gamma)$ collects, for each rule in $\Gamma$, every
subformula occurring in the implication-free temporal
...
\begin{lemma}\label{lemma:bij-helper}
  ...
  fully determined by its values on atoms
...
\begin{proof} We will prove each assertion in order.
  ...
  denote its associated 3 valued evaluation functions in the
...
\begin{proof} We break the proof down into the following steps:
  ...
  $m \in \text{THT}(\Gamma,\lambda)$ some $I \in \text{HT}(\chi(\Gamma))$
...
its stable models. Since we have $m_I(k,a)=I(L_a^k)$ for any
...
For a temporal logic program $\Gamma$ we define its translation to a logic
