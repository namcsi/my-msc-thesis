...
its \emph{gringo representation}.
...
telingo, its namesake, as telingo only accepts so-called
...
Given an input temporal logic program (in its gringo representation),
referred to as tlp.lp we can obtain its temporal stable models by
...
to its symbolic representation \verb|O| (a symbolic function in the
...
formulas and hold at time point \verb|K|; this behavior is disabled
...
\verb|since|. So, assuming the domain we ground the program over its
